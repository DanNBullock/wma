function [fg_classified,fg_unclassified,classification,fg] = wma_majortracts(dt6_file, fg, Atlas, ...
    ...
    for roiID = 1:size(moriRois, 1)
        ...
        if recomputeROIs
            ...
            if exist('antsInvWarp','var') && ~isempty(antsInvWarp)
                ...
                [~, invDef, roi1(roiID)]=dtiCreateRoiFromMniNifti(dt6_file, ROI_img_file, invDef, true);
            end
        else
        ...
        if recomputeROIs
            [~, invDef, ~] = dtiCreateRoiFromMniNifti(dt6_file, ROI_img_file, invDef, true);
            % Default is to use the spm normalization unless a superior
            % ANTS normalization was passed in
            if exist('antsInvWarp','var') && ~isempty(antsInvWarp)
                ...
                [~, invDef, roi2(roiID)] = dtiCreateRoiFromMniNifti(dt6_file, ROI_img_file, invDef, true);
            end   
        else
            RoiFileName = fullfile(fileparts(dt6_file), 'ROIs',  [prefix(prefix(ROI_img_file, 'short'), 'short') '.mat']);
...
for ii = 1:size(moriRois, 1)
    fg_classified(ii) = wma_reorient_fibers(fg_classified(ii),roi1(ii),roi2(ii));
end
