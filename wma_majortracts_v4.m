function [classification] = wma_majortracts_v4(dt, fg, Atlas, ...
...
%
% Input parameters:
% dt6_file                 - A path to a dt6.mat file created to by
%                            dtiInit.m
...
% E.g., to avoid recomputing ROIs and use minDist of 4mm one would pass
...
%       We avoid this because it changes the fiber grou (FG) and creates 
%       conflics with the FE (LiFE structure).
...
if useRoiBasedApproach 
    ...
    [~, ~, InterHemisphericFibers] = dtiIntersectFibersWithRoi([], 'not', [], midSaggitalRoi, fg); 
    ...
    for roiID = 1:size(moriRois, 1)
        ...
        if recomputeROIs
            ...
            if exist('antsInvWarp','var') && ~isempty(antsInvWarp)
                outfile = fullfile(fileparts(dt6_file),'ROIs',moriRois{roiID, 1});
            ...
            RoiFileName=fullfile(fileparts(dt6_file), 'ROIs',  [prefix(prefix(ROI_img_file, 'short'), 'short') '.mat']);
            roi1(roiID) = dtiReadRoi(RoiFileName);  
        end
        % Find fibers that intersect the ROI
        [~,~, keep1(:, roiID)] = dtiIntersectFibersWithRoi([], 'and', minDist, roi1(roiID), fg);
        ...
        if recomputeROIs
            ...
            % ANTS normalization was passed in
            if exist('antsInvWarp','var') && ~isempty(antsInvWarp)
                outfile = fullfile(fileparts(dt6_file),'ROIs',moriRois{roiID, 2});
...
%% NOW FIX IT
%standardized method for removing tracts
removeTracts={'ILF','SLF'};
for iRemove=1:length(removeTracts)
    targetInd=strfind(names,removeTracts{iRemove});
    for iLabels=1:length(targetInd)
        removeBool(iLabels)=~isempty(targetInd{iLabels});
    end
    removeThese=find(removeBool);
    
    for iRemoveHere=1:length(removeThese)
        removeIND=removeThese(iRemoveHere);
        fprintf('\n removing Old %s from classification', names{removeIND})
        names(removeIND)=[];
        fiberIndex(fiberIndex==removeIND)=0;
        minusVec=(fiberIndex>removeIND)*-1;
        fiberIndex=fiberIndex+minusVec;
        if removeThese(end)~=removeThese(iRemoveHere)
        if removeThese(iRemoveHere+1)>removeThese(iRemoveHere)
            removeThese(iRemoveHere+1)=removeThese(iRemoveHere+1)-1;
        end
        end
    end
    clear removeBool
