function  [nii, nii2, nii_normalized, nii2_normalized] = csc_endpointMaps_Decay_v4(fg, fsDir,saveDir, saveHeader, thresholdinmm, outlierreject, decayFunc)
...
fname{1} = strcat(fg.name,'_',decayFunc,'_',num2str(thresholdinmm),'mm_',orientation1,'FiberEndpoint.nii.gz'); % Tract endpoint density without normalization
fname{2} = strcat(fg.name,'_',decayFunc,'_',num2str(thresholdinmm),'mm_',orientation2,'FiberEndpoint.nii.gz');
saveFileName{1} =  strcat(fg.name,'_',decayFunc,'_',num2str(thresholdinmm),'mm_',orientation1,'FiberEndpoint_normalized.nii.gz'); % Tract endpoint density with normalization
saveFileName{2} =  strcat(fg.name,'_',decayFunc,'_',num2str(thresholdinmm),'mm_',orientation2,'FiberEndpoint_normalized.nii.gz');
...
for iFibBins=1:length(fiberBins)-1
    ...
    switch decayFunc
        ...
        case 'linear'
            ...
            % distance of zero =1 and a distance approaching the threshol
...
niftiWrite(nii,fullfile(saveDir,fname{1}));
niftiWrite(nii2,fullfile(saveDir,fname{2}));
